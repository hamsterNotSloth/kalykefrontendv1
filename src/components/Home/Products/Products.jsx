...
function Products() {
  const { filter, category, isFree } = useSelector(state => state.filtersSlice)
  const { data: productsData, refetch: getAllProducts, isError, isFetching, isLoading } = useGetAllProductsQuery({filter, category, isFree})
  useEffect(() => {
    getAllProducts({filter,  category, isFree})
  }, [filter, category, isFree])
